...
## 기술 스택 (절대 규칙) `[확인필요: Week 1 07_DECISIONS.md에서 최종 확정]`
...
5. `../docs/06_VOICE_AND_TONE.md` — 화면 성격(사장 화면 vs 직원 현장 화면)에 따라 문구 톤이 다르다
...
2. **04_DATA_CONTRACT.md(shared)와 05_DATA.md를 컨텍스트로 항상 포함할 것.**
...
- 04_DATA_CONTRACT.md에 없는 데이터를 org로 보내야 할 것 같을 때
...
> ⚠️ `00_PROJECT_START` / `01_MUST_PREPARE` / `02_BEFORE_CODE`는 여기 두지 않는다 — 같은 역할을 이미 `../docs/00_PRODUCT_GUIDE.md`, `01_PERSONAS.md`(shared)가 하고 있어서 중복이기 때문이다. 프로젝트 배경·페르소나·스코프가 궁금하면 그쪽을 먼저 읽을 것.

## 문서 소유권 원칙

이 CLAUDE.md를 포함한 `TongssApp/docs/` 전체의 **최종 수정 권한은 Store PO**에게 있다. PM(Sara)은 초안과 가이드라인을 제시하지만, 확정·변경은 Store PO의 몫이다. 트랙을 넘는 결정(데이터 계약 변경 등)은 `../docs/07_DECISIONS.md`에 기록.