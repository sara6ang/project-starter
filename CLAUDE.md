...
> 👋 **사람이 처음 합류했다면** 이 파일보다 `docs/00_01_START_HERE.md`를 먼저 읽으세요 — 이번 주에 뭘 읽고 뭘 생각해봐야 하는지 안내합니다.
...
`docs/`(공유)는 아래 8개 문서로 구성됩니다. **"팀원이 10분 안에 읽고 바로 작업을 시작할 수 있는 문서"가 목표라 짧고 실용적으로 유지합니다.** 개발과 무관한 조사 자료(재무 분석, 경쟁사 리서치, 인터뷰 메모 등)는 `archive/`에 따로 둡니다.

> `00_01_START_HERE.md`는 이 번호 체계 밖입니다 — 08개 문서 중 하나가 아니라, **그 문서들을 "이번 주에 이 순서·이 관점으로 읽으라"고 안내하는 주차별 온보딩 문서**입니다. 파일명이 `00_PRODUCT_GUIDE.md`보다 알파벳상 먼저 오도록 의도적으로 지었습니다 — `docs/` 폴더를 열었을 때 제일 먼저 보이는 파일입니다.
...
| 03 | PROJECT_GUIDE — 일정 + 팀 협업 규칙 (로드맵 + 팀 가이드) |
| 04 | DATA_CONTRACT — 두 시스템 간 데이터 정의 |
| 05 | PROJECT_TREE — 리포지토리 전체 큰 그림 |
| 06 | VOICE_AND_TONE — UX 라이팅 원칙 |
| 07 | DECISIONS — 의사결정 기록 (전 스택 공통) |

⚠️ **TongssApp/TongssOrg 스택 docs(03~07번)는 위 공유 번호와 더 이상 1:1로 대응하지 않습니다** (예: 스택 03_USER_FLOW는 공유 02_USER_FLOW와 대응, 스택 07_DESIGN_SYSTEM은 공유 06_VOICE_AND_TONE과 대응). 2026-07-27 리팩토링에서 공유 docs만 "일하기 위한 문서" 기준으로 재구성했고, 스택별 docs 번호 체계는 각 트랙 리드(아론/은영)가 필요하다고 판단할 때 별도로 정리합니다. 헷갈리면 번호보다 **문서 제목**으로 찾으세요.

---

## AI 툴 사용 규칙 (요약 — 전체는 `docs/03_PROJECT_GUIDE.md`, Manifesto도 그 문서 안)

1. **작업 시작 전 docs 먼저.** 코드를 시키기 전에 해당 스택의 CLAUDE.md와 관련 docs(특히 04_DATA_CONTRACT)를 컨텍스트로 제공하세요.
...
5. **스코프 판단 기준은 Demo Day 한 줄 시나리오입니다** (`00_PRODUCT_GUIDE.md` §3 참조): "사장이 매뉴얼 등록 → 직원이 봄 → 토스 쪽이 org에서 확인". 여기 기여하지 않는 기능은 우선순위가 아닙니다.
