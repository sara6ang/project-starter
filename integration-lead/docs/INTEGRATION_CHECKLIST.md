...
- [ ] store-app 배포처 확정 (04_ROADMAP Week 1 필수 결정)
- [ ] toss-org에서 Account 커스텀 필드 생성 완료 (Org PO)
- [ ] Digital Experience Site 활성화 여부 확인

## Hello World 스파이크 (Week 1 후반)

- [ ] 더미 값 1개를 store-app에서 하드코딩해 POST
- [ ] Apex REST(`StoreRestService`)가 받아서 Account 필드 갱신 확인
- [ ] Guest User 권한으로 실제 호출 성공 확인 (로그인 세션 없이)
- [ ] CORS 에러 발생 여부 확인 → 발생 시 Allowed Origins에 store-app 도메인 등록
...
- [ ] 엔드투엔드: store-app 실제 활동 → org 레코드 반영까지 실 데이터로 확인
