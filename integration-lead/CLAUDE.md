...
> 루트 규칙은 `../CLAUDE.md` 참조. 이 폴더는 `docs/06_PROJECT_TREE.md`에서 설명하듯, store-app·toss-org 어느 한쪽에도 속하지 않는 **연동 자체에 대한 기록**을 위한 공간입니다.

---

## 이 폴더가 무엇인가

store-app ↔ toss-org 데이터 연동(Apex REST, Guest User, CORS)의 상태·이력·환경 정보를 모아두는 곳. 실제 코드는 여기 없다 — store-app의 API 호출 코드는 `store-app/`에, Apex REST 클래스는 `toss-org/`에 있다. 여기는 **그 연동을 다루는 문서**만 둔다.

작업 전에 먼저 읽을 것: `../docs/05_DATA_CONTRACT.md`(shared, 필드 계약의 유일한 진실), `store-app/docs/06_ARCHITECTURE.md`, `toss-org/docs/06_ARCHITECTURE.md`(외부 통합 아키텍처 섹션).
...
1. **가급적 VS Code + Claude Code 확장을 사용할 것.** 이 역할은 store-app 저장소와 toss-org 저장소(force-app)를 오가며 작업하므로, 두 프로젝트를 각각 열어 diff를 확인할 수 있는 VS Code가 특히 유리하다.
