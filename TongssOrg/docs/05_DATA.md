...
> ⚠️ 이 문서는 org 내부 스키마 전체를 정의한다. TongssApp에서 넘어오는 필드의 유일한 진실은 `../../docs/05_DATA_CONTRACT.md`(shared)다 — **다른 문서다, 헷갈리지 말 것.**
...
| 매뉴얼/체크리스트 **원본 데이터는 org에 저장하지 않음** — 집계 필드만 Account에 | 02_PRD 스코프상 org는 "상태 확인"만 하면 됨. 원본을 다 옮기면 데이터 계약이 비대해지고 TongssApp과 이중 관리됨 |
...
| Store ID | `Store_Id__c` | Text (External ID) | TongssApp의 `store_id`와 매핑되는 키 — **05_DATA_CONTRACT §2 참조, 유일한 연결고리** |
...
Pepper's Oven 때보다 훨씬 단순하다 — 자식 오브젝트(Order__c, Inventory_Consumption__c 같은 로그성 데이터)가 없다. 이유: 이번 프로젝트의 org는 "기록을 남기는 시스템"이 아니라 "TongssApp의 상태를 비추는 거울"이기 때문 (02_PRD §1 참조).
...
- 대안: TongssApp이 `is_active`를 직접 계산해서 보내는 방식(05_DATA_CONTRACT §3-5) — Flow보다 단순하지만 로직이 TongssApp에 종속됨
