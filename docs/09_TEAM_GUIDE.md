...
> 최종 수정: 2026-07-27 (Manifesto 추가) / 오너: Sara

---

## 0. Manifesto

1. **People own features. PM owns the project. Everyone owns quality.** 각자 트랙의 결정권은 트랙 리드에게. PM은 트랙 안의 결정을 대신 하지 않는다.
2. **Every feature has one owner.** 애매하게 걸쳐있는 기능이 없게 한다.
3. **AI writes first. Humans improve.** 초안·리뷰·테스트케이스·릴리스노트·Mermaid·Flow·네이밍은 AI가 먼저 만든다.
4. **No Docs, No Decision.** 문서 없는 결정은 결정이 아니다.
5. **Keep the architecture simple.** 처음부터 화려하게 만들지 않는다. 필요해지면 그때 확장한다.

---

## 1. PM vs Team Lead — 역할 분리
...
| **주간 산출물** | 로드맵 갱신, 08_DECISIONS 기록, 스코프 조정, **일일 스탠드업 진행** | git 저장소 상태(브랜치·머지) 점검 |
...
| git 저장소 운영 | 은영 (Team Lead) | 전원 | 브랜치 보호, 프리즈 규칙 집행, 트랙 간 머지 충돌 조율 |
...
- **진행은 Sara(PM)가 맡는다** (부담 조정 — 은영의 상시 업무 부담을 줄이기 위해 Sara가 계속 진행하기로 함. 09_TEAM_GUIDE §1 문제유형별 표의 "팀 운영 이슈는 Team Lead" 원칙과는 별개로, 스탠드업 진행이라는 실행 그 자체는 Sara가 맡는 것으로 조정).
- 격일이 아니라 매일인 이유: 전원 E 성향 팀은 안 모이면 트랙에 갇혀 각자 폭주할 위험이 있고(§8 30분 룰), 격일로는 막힘을 늦게 발견한다. 특히 통합 주간(Week 3)엔 매일 체크가 사실상 필수.
...
- 승우가 벽(권한/CORS류)을 만나면 혼자 두지 않는다. 혜준이 1번 지원, Sara가 Pepper's Oven 경험으로 조언, 나머지는 즉시 투입 가능 상태 유지. **트랙 간 조율·전원 소집은 Sara(PM)가 맡는다** (부담 조정 — 은영에서 이관, Sara가 이 주간 이미 이슈 트리아지를 맡고 있어 자연스럽게 통합).
